...
export function validateEnvironment(config: EnvironmentConfig): void {
  ...
  for (const key of Object.keys(config.variables)) { const value=config.variables[key]??''; if (secretPattern.test(key) && value.length < 8) throw new Error(`WEAK_SECRET:${key}`); }
